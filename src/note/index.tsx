...
const Note = React.forwardRef(({note: original , abc }: any, ref: any,) => {
        ...
        if (isEditing) {
            ...
            const endEditing = (e: any) => {
                ...
                if (note.title !== titleNode.current.innerText || note.description !== descNode.current.innerText) {
                    fetch("https://nogic-apis.herokuapp.com/api/edit_note.php", {
                        method: "PUT",
                        headers: {
    ...
    const deleteNoteTimeout = () => {
        fetch("https://nogic-apis.herokuapp.com/api/delete_note.php?id=" + note.id, {
            method: "DELETE",
            headers: {
                "Content-Type": "application/json"
            }
    ...
    const colorBtnCallback = (color: string,colorCode: string) => {
        fetch("https://nogic-apis.herokuapp.com/api/edit_note_color.php", {
            method: "PUT",
            headers: {
                "Content-Type": "application/json"
            },
